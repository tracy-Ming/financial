...
class MyAccount:
    def __init__(self,args):
        ...
        self.lever = 500
        self.cost = 0.035
        self.swap = 0.01 / 60 / 24 / self.lever * (500 * 1000)  ### each day interest rate is 0.01 for 0.01 lot and 500..
    ...
    def CloseAskOrders(self,dprice):
        rw = 0.0
        for ask_order in self.AskOrders:
            # ask_order["profit"] + dprice  means each askorder's total profits
            rw = rw + ask_order["lots"] * self.standard_order * ( ask_order["profit"] + dprice ) * self.lever - self.cost
        self.Account = self.Account + rw
        self.AskOrders = []
        return rw

    def CloseBidOrders(self,dprice):
        rw = 0.0
        for bid_order in self.BidOrders:
            rw = rw - bid_order["lots"] * self.standard_order * ( bid_order["profit"] + dprice ) * self.lever - self.cost
        self.Account = self.Account + rw
        self.BidOrders = []
        return rw
