...
nql={}
nql=torch.class('dqn.NeuralQLearner')

function nql:myinfo()
 print("Started agent initing...")
end
...
--    self.best       = args.best
    ...
    self.input_dims     = args.input_dims or  {1, self.state_dim, 1}
    ...
    if args.network=='trainning_dqn' then
        self.network    = cnn_config(args)
    else
          local msg, err = pcall(require, args.network)
          if not msg then
              -- try to load saved agent
              local err_msg, exp = pcall(torch.load, args.network)
              if not err_msg then
